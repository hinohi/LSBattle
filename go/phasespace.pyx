...
from matrix44 cimport Matrix44, mat4

from vector4D import Vector4D, vec4
from matrix44 import Matrix44, mat4
...
cdef class PhaseSpace(object):
    ...
    def transform(self, Vector4D acceleration, double ds):
        ...
        self.X._x = self.X._x + self.U._t * ds
        self.X._y = self.X._y + self.U._t * ds
        self.X._z = self.X._z + self.U._t * ds
        acceleration._t = 0.0
        cdef Matrix44 L = mat4.Lorentz(-self.U)
